...
export default function Batch() {
  ...
  const actions: AppAction[] = [
    ...
    { id: 'run', label: 'Executar', variant: 'primary', disabled: busy },
  ...
  const onAction = async (id: AppActionId) => {
    ...
    if (id === 'run') {
      try {
        setBusy(true);
        if (b.mode === 'fsa') await b.runFsa();
        else await b.runZipFallback();
  ...
  return (
    <div className="page">
      <div className="card form">
        ...
        <p className="muted">Processa cada arquivo de entrada aplicando User Template + Regex e envia à LLM.</p>
        ...
        <section className="group">
          ...
          <div className="grid-2">
            <div className="field">
              ...
              <input type="number" min={1} className="input" value={b.cfg.concurrency}
                onChange={(e) => b.setPartialCfg({ concurrency: Math.max(1, parseInt(e.target.value || '1', 10)) })} />
            </div>
            <div className="field">
              <label>Sufixo do arquivo de saída</label>
              <input className="input" value={b.cfg.outputSuffix} onChange={(e) => b.setPartialCfg({ outputSuffix: e.target.value })} />
            </div>
            <div className="field">
              <label>Limite de erros para abortar</label>
              <input type="number" min={1} className="input" value={b.cfg.abortAfterErrors}
            ...
            <div className="field">
              ...
              <input type="number" min={1} className="input" value={b.cfg.maxFileSizeMB}
          ...
          <div className="radio-row">
            <label className="radio">
              <input type="checkbox" checked={b.cfg.overwrite} onChange={(e) => b.setPartialCfg({ overwrite: e.target.checked })} />
              <span>Sobrescrever arquivos existentes (FSA)</span>
            </label>
            <label className="radio">
              <input type="checkbox" checked={b.cfg.failOnError} onChange={(e) => b.setPartialCfg({ failOnError: e.target.checked })} />
    ...
    <section className="group">
      ...
      <div className="fsa-row">
        <div>
          ...
          <button className="btn btn-secondary" onClick={b.chooseInputDir}>Selecionar</button>
          <div className="hint">{b.inputDir ? 'Selecionado' : '—'}</div>
        </div>
        <div>
          <div className="muted">Saída</div>
          <button className="btn btn-secondary" onClick={b.chooseOutputDir}>Selecionar</button>
...
function ZipSection() {
  ...
  return (
    <section className="group">
      ...
      <input type="file" multiple onChange={(e) => e.target.files && b.chooseFilesFallback(e.target.files)} />
